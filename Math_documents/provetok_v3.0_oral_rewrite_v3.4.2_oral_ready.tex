...
We study 3D CT report generation under an explicit \emph{evidence budget} $B$.
\textbf{$B$ is the size of a fixed per-volume token bank:} a frozen 3D encoder is run once to construct at most $B$ spatial evidence tokens (each with a 3D bbox + feature), and \emph{all} sentences may only route from this same bank (no additional evidence is retrieved during generation).
We then enforce \textbf{sentence-level evidence commitment} via \textbf{token-gated generation}:
each sentence is generated with only a small routed subset of $k\ll B$ tokens, and the routed token IDs become the sentence citations.
...
\paragraph{Why an explicit budget matters (oral hook).}
A central empirical question is whether allocating ``more evidence tokens'' actually increases faithfulness.
In global-prefix baselines, evidence is merely \emph{available}: the model may ignore the visual channel, and increasing $B$ may \emph{dilute} attention, so faithfulness need not be monotonic in $B$; we therefore measure the faithfulness--budget curve explicitly via controlled budget sweeps.
We therefore treat $B$ as a first-class variable and sweep it systematically, reporting verifier pass rate and counterfactual sensitivity as a function of $B$ under matched LLM-token compute.
Token-gated generation turns evidence into a \emph{hard interface}: each sentence must commit to $k$ localized tokens, and these token IDs define the citations.


\subsection{Core Claims (Oral-Ready)}
\begin{itemize}[nosep]
\item \textbf{C1 (Main thesis): Evidence \emph{commitment} interface.}
Turn evidence from \emph{available} to \emph{mandatory}: every sentence must \emph{commit} to $k$ localized tokens drawn from a fixed bank of size $B$, and these token IDs are emitted as citations.
...
\noindent\textbf{Note on the opening hook.}
Our core claim does \emph{not} require a strictly non-monotonic faithfulness--budget curve:
we will characterize the faithfulness--budget trend via controlled $B$ sweeps and, regardless of whether faithfulness saturates or degrades with larger $B$, test whether sentence-level commitment (gating) improves \emph{audited faithfulness} at matched compute and fixed $L$.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Observation / question:} with a single global visual prefix, evidence is optional; as we increase the token-bank budget $B$ (tokens exposed for the whole volume), attention may dilute, so we explicitly test whether audited faithfulness is monotonic in $B$.
...
\begin{itemize}[nosep]
    \item \textbf{Budgeted token bank construction} (adaptive split, artifact gating, boundary caching) --- an efficient way to produce $B$ spatial tokens from frozen features.
...
\paragraph{Stage 2: adaptive octree split under budget $B$.}
Starting from a coarse grid ($d_0{=}2$, giving $4^3{=}64$ initial cells on a $128^3$ volume), we iteratively score cells by a fixed, deterministic rule (uncertainty + boundary + optional semantic prior) and split the highest-scored cells until reaching the evidence budget $B$ (or max depth $d_{\max}{=}5$).
...
\paragraph{Why ``more tokens'' may not help.}
Simply increasing $B$ does not force evidence usage: with a global prefix, additional tokens can dilute attention and increase shortcutting, so faithfulness may be \emph{non-monotonic} in $B$ (a question we test via budget sweeps).
Token gating makes the evidence channel \emph{obligatory} at the sentence level, enabling interpretable budget--faithfulness curves.


Token-gated generation turns evidence usage into a \emph{hard interface}:
for each sentence, only a small routed subset of tokens enters the prompt, which forces every sentence to be conditioned on explicit spatial evidence.
...
\midrule
$k$ (tokens per sentence) & 8 & $\{4, 8, 16, 32\}$ & Routing budget per sentence \\
...
\begin{table}[H]
\centering\small
\caption{Main results on 5K test split (500 cases, 3979 sentences). B2'v2 = official model.}
\begin{tabular}{lccc}
\toprule
...
\begin{itemize}[nosep]
\item \textbf{B2'v2 $\to$ E1:} Switching from anatomy-primary (IoU-dominant) to semantic-primary (trained $W_{\mathrm{proj}}$ + cosine rerank) trading R1 for R3.
...
\subsection{Supplementary Ablations (Appendix)}
...
\midrule
S1 & Token bank construction & Adaptive split vs.\ fixed-octree vs.\ uniform grid \\
S2 & Routing budget $k$ & $k \in \{4, 8, 16, 32\}$ sensitivity curve \\
S3 & Evidence Card version & v1 (80\% threshold) vs.\ v2 strict (90\% + min 2 tokens) \\
S4 & Stage 5 judge effect & With vs.\ without LLM judge filtering \\
S5 & Boundary caching & $\beta{=}0.1$ vs.\ $\beta{=}0$ vs.\ $\beta{=}0.2$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Factorial design (location vs.\ content).}

\begin{table}[H]
\centering\small
\caption{Counterfactual experiments to validate evidence usage.}
\begin{tabular}{lp{5.2cm}p{5.0cm}}
\toprule
Experiment & Intervention & Expected outcome \\
\midrule
Content-scramble & Permute token features while keeping bboxes fixed & Faithfulness drops if model uses evidence content \\
Wrong-location & Keep features but swap bboxes to incorrect region & Laterality/anatomy violations increase \\
Wrong-region routing & Force routing to spatially incorrect tokens & R1/R2 violations spike \\
Cross-patient & Use patient A's tokens for patient B & Severe degradation \\
Token-drop & Replace top-$k$ with low-score tokens & Graded quality degradation \\
Random-router & Uniform random token sampling & Validates routing necessity \\
\bottomrule
\end{tabular}
\end{table}
