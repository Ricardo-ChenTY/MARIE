...
\begin{table}[htbp]
\centering
\caption{与外部基线的报告生成质量对比。基线结果引用自各方法原始论文（published protocol）；ProveTok 使用我们的 5K 划分（\texttt{seed=42}，train/valid/test = 2000/250/250）。$\uparrow$ 表示越高越好。}\label{tab:main_comparison}
\small
\begin{tabular}{@{}l l l r r r@{}}
\toprule
\textbf{Dataset} & \textbf{Method} & \textbf{Protocol} & \textbf{BLEU-4}$\uparrow$ & \textbf{METEOR}$\uparrow$ & \textbf{ROUGE-L}$\uparrow$ \\
\midrule
CT-RATE & CT2Rep              & published protocol & 0.172 & 0.173 & 0.243 \\
CT-RATE & CT-AGRG             & published protocol & 0.172 & 0.196 & 0.280 \\
CT-RATE & \textbf{ProveTok (ours)} & our split (\texttt{seed=42}, 2000/250/250) & \textbf{0.467} & \textbf{0.603} & \textbf{0.626} \\
\midrule
RadGenome-ChestCT & MedVInT  & published protocol & 0.246 & 0.404 & 0.326 \\
RadGenome-ChestCT & Reg2RG   & published protocol & 0.249 & 0.441 & 0.367 \\
RadGenome-ChestCT & \textbf{ProveTok (ours)} & our split (\texttt{seed=42}, 2000/250/250) & \textbf{0.506} & \textbf{0.660} & \textbf{0.680} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Grounding 指标（5K test split）。Overlap = 均值重叠率；Hit@$k$ = top-$k$ 命中率；RP = Routing Precision；Lat.Acc = 侧别准确率；CF = Citation Faithfulness；DF = Depth Faithfulness。``---'' = 该配置无 LLM 生成，无法计算。}\label{tab:grounding}
\small
...
\toprule
\textbf{Config} & \textbf{Viol.\%} & \textbf{CI$_{\mathrm{lo}}$} & \textbf{CI$_{\mathrm{hi}}$} & $n$ \\
\midrule
A0     & 7.59 & 6.84 & 8.35 & 496 \\
A1     & 5.96 & 5.27 & 6.64 & 496 \\
E1     & 6.04 & 5.34 & 6.77 & 496 \\
B2$'$  & 5.85 & 5.14 & 6.60 & 496 \\
\textbf{B2$'$v2} & \textbf{4.19} & \textbf{3.60} & \textbf{4.83} & \textbf{496} \\
C2$'$  & 6.00 & 5.25 & 6.79 & 496 \\
D2     & 5.79 & 5.07 & 6.55 & 496 \\
\bottomrule
\end{tabular}
\end{table}
