...
以下表格基于 5K 数据集（CT-RATE 250 例 + RadGenome-ChestCT 250 例，共 496 个病例、3{,}979 个句子）的 test split 结果。

%─────────────────────────────────────────────────────────
\subsection{Table 2：消融链主表——空间一致性与计算开销}
%─────────────────────────────────────────────────────────

\begin{table}[htbp]
\centering
\caption{消融链主表（5K test split，496 例，3{,}979 句）。Viol.\% 为句子级违规率；R1/R3/R6b 为各规则违规计数；$C_{\mathrm{LLM}}$ 为 LLM 调用次数；$N_{\mathrm{reroute}}$ 为重路由次数。}\label{tab:ablation_main}
\small
...
\begin{table}[htbp]
\centering
\caption{Grounding 指标（5K test split）。Overlap = 均值重叠率；Hit@$k$ = top-$k$ 命中率；RP = Routing Precision；Lat. Acc = 侧别准确率；CF = Citation Faithfulness；DF = Depth Faithfulness。``---'' 表示该配置无 LLM 生成，无法计算。}\label{tab:grounding}
\small
...
%─────────────────────────────────────────────────────────
\subsection{Table 4：NLG 质量指标}
%─────────────────────────────────────────────────────────

\begin{table}[htbp]
\centering
\caption{NLG 质量指标（5K test split）。仅有 LLM 生成的配置（B2$'$ 及之后）可计算。}\label{tab:nlg}
\small
...
\begin{table}[htbp]
\centering
\caption{相邻配置 Permutation Test + Holm 校正（$R=5{,}000$，$\alpha=0.05$）。$\Delta$ 为 Viol.\% 差值（百分点）；$^*$ 表示 Holm 校正后显著。}\label{tab:pairwise}
\small
...
\paragraph{主要发现。}
\begin{enumerate}[nosep]
    \item \textbf{B2$'$v2 为最优配置}：Viol.\% = 4.25\%，95\% CI [3.60\%, 4.83\%]，CI 上界与其余配置 CI 下界不重叠，统计显著。
