...
\begin{itemize}[nosep]
    ...
    \item \textbf{参数来源：}$\lambda_{\mathrm{spatial}} = 0.3$ 来自 CT-GLIP grounded VLP 的经验值。在解剖优先模式~\eqref{eq:anat_primary} 中，$\epsilon = 0.05$ 确保同 IoU token 之间仍有语义分辨力。
\end{itemize}
\end{definition}
...
\paragraph{输入 / 输出 / 参数来源。}
\begin{itemize}[nosep]
    \item \textbf{输入（训练数据）：}通过 bootstrap 策略构建——先用 $W_{\mathrm{proj}} = I$ 跑一遍完整流水线，从 \texttt{trace.jsonl} 中提取 (句子 $s$, cited token IDs $\{t_1,\ldots,t_k\}$) 配对。正例特征 $\bar{f}_s^+ = \frac{1}{k}\sum_{j=1}^{k} f_{t_j}$（cited token 均值）。批内其它句子的正例作为负样本（in-batch negatives）。
    \item \textbf{输出：}训练后的 $W_{\mathrm{proj}}$ — 使视觉嵌入 $v_i$ 与语义相关的文本查询 $q_s$ 在 $\RR^{384}$ 空间中对齐。
