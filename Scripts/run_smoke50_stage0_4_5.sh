...
DONE_FLAG="${ROOT}/logs/smoke50.done"
...
python "${ROOT}/Scripts/ckpt_probe.py" \
  ...
  --out_channels 2 \
  --feature_size 48 \
  --save_report "${OUT4}/ckpt_probe_report.json" 2>&1 | tee "${OUT4}/run.log"
...
python "${ROOT}/run_mini_experiment.py" \
  ...
  --r1_negation_exempt \
  --r1_skip_midline \
  --r1_min_same_side_ratio 0.6 2>&1 | tee -a "${OUT4}/run.log"

python "${ROOT}/validate_stage0_4_outputs.py" \
  --out_dir "${OUT4}" \
  --datasets ctrate,radgenome \
  --expected_cases_map ctrate=50,radgenome=50 \
  --save_report "${OUT4}/validation_report.json" 2>&1 | tee -a "${OUT4}/run.log"

printf "[%s] DONE stage0_4_50\n" "$(date -Is)" | tee -a "${MASTER_LOG}"
printf "[%s] START stage0_5_50\n" "$(date -Is)" | tee -a "${MASTER_LOG}"
python "${ROOT}/Scripts/ckpt_probe.py" \
  --ckpt_path "${ROOT}/checkpoints/swinunetr.ckpt" \
  --in_channels 1 \
  --out_channels 2 \
  --feature_size 48 \
  --save_report "${OUT5}/ckpt_probe_report.json" 2>&1 | tee "${OUT5}/run.log"
...
python "${ROOT}/run_mini_experiment.py" \
  ...
  --llm_judge_alpha 0.5 2>&1 | tee -a "${OUT5}/run.log"

python "${ROOT}/validate_stage0_4_outputs.py" \
  --out_dir "${OUT5}" \
  --datasets ctrate,radgenome \
  --expected_cases_map ctrate=50,radgenome=50 \
  --save_report "${OUT5}/validation_report.json" 2>&1 | tee -a "${OUT5}/run.log"
