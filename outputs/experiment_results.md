...
| 图表 | 文件 | 内容 |
|------|------|------|
| Fig 1 — 瀑布图 | `paper_figures_5k/fig1_waterfall.{pdf,png}` | 消融链 Viol% + R1/R3 分解 |
| Fig 2 — k-sweep | `paper_figures/fig2_budget_sweep.{pdf,png}` | k vs 违规率 |
| Fig 3 — Counterfactual | `paper_figures_5k/fig3_counterfactual.{pdf,png}` | 扰动敏感性 |
| Fig 4a — 正例 | `paper_figures/fig4a_qualitative_positive.{pdf,png}` | 零违规案例 |
| Fig 4b — 反例 | `paper_figures/fig4b_qualitative_negative.{pdf,png}` | R1 违规案例 |
| Fig 4 — 组合 | `paper_figures/fig4_qualitative_combined.{pdf,png}` | 正反例组合 |
| Fig 5 — 对比 | `paper_figures/fig5_comparative.{pdf,png}` | A0 vs B2'v2 同一 case |
| Bootstrap CI | `statistical_analysis/bootstrap_ci_forest.{pdf,png}` | CI forest plot |
| Rule 分布 | `statistical_analysis/rule_stacked_bar.{pdf,png}` | 规则堆叠条形图 |
| Anatomy 违规 | `statistical_analysis/anatomy_violations.{pdf,png}` | 按解剖区域分布 |
