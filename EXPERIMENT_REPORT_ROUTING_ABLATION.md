...
> 实验日期：2026-03-15
> 输出目录：`outputs/ablation_routing_2x2/`（Phase A + E1）、`outputs/ablation_k_sweep/`（k sweep）
...
所有实验统一：180 test cases（90 CT-RATE + 90 RadGenome），Stage 3c OFF，Stage 5 OFF（gold text），`token_budget_b = 128`。
...
|------|--------|---------|:----------:|:----:|
| A0 | identity | spatial | 208 | 14.5% |
| **A1** | trained | spatial | **189** | **13.2%** |
...
- **R3 大幅下降（80→19，−76%）**：语义 rerank 选择了深度层级更匹配的 token。空间路由只看 IoU 不看 level，而 E1 先过滤 level 范围，再用语义选择。
- **R1 上升（92→128，+39%）**：语义 rerank 在空间合格 token 中选择了更多跨侧 token。这可能是因为语义相关但位于对侧的 token 被优先选中。
...
- **k=2 异常点**：R1=187 显著高于相邻 k 值。推测当 k=2 时，top-2 token 恰好一左一右触发 laterality 违规。
...
| 组件 | 配置 |
|------|------|
| W_proj | Trained（InfoNCE） |
| 路由模式 | E1：Spatial filter + Semantic rerank |
| k_per_sentence | 8 |
| τ_iou | 0.04 |
| Stage 3c | OFF（待 Phase B 验证） |
| Stage 5 | OFF（待 Phase C 验证） |

### 5.2 违规率演进

| 阶段 | 违规数 | Rate | 相对变化 |
|------|:------:|:----:|:--------:|
| A0 (identity + spatial) baseline | 208 | 14.5% | — |
| A1 (+ trained W_proj) | 189 | 13.2% | −9.1% |
| E1 (+ filter+rerank) | 164 | 11.4% | −21.2% |

从初始 baseline 到 E1，总违规下降 **21.2%**。

### 5.3 后续计划

| Phase | 内容 | 依赖 |
|-------|------|------|
| B2 | E1 + Stage 3c ON（LLM token-gated 生成） | 需要 LLM |
| C2 | B2 + Stage 5 ON（LLM 验证+重路由） | 需要 LLM |

这将验证 E1 路由改进是否在完整 pipeline 中传递到最终生成质量。
