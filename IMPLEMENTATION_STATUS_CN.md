...
|-------------|------|---------|------|
| **Eq(11)** | `C_no-cache = Σ_j (p_j + g_j)²` | **未实现** | 需要记录每句 prompt 长度 p_j 和生成 token 数 g_j |
| **Eq(12)** | `C_cache = Σ_j (p²_j + g_j·p_j + g_j(g_j-1)/2)` | `analyze_outputs.py` 用 `C_attn = k·B`（简化代理） | 公式不同：当前用 k·B 近似，没有考虑实际 prompt/生成长度 |

**修复方案**：
1. 在 `stage3c_generator.py` 的 `generate_sentence()` 中记录 prompt token 数和生成 token 数
2. 在 trace.jsonl 的 sentence 条目中写入 `prompt_tokens` 和 `gen_tokens`
3. 在 `analyze_outputs.py` 中按 Eq(12) 计算 `C_cache`，同时保留 `k·B` 作为轻量参考
...
| 项目 | Overleaf | 实现 |
|------|---------|------|
| C_LLM | 总 LLM 调用次数 | `C_LLM = n_gen + n_judge` — 包含 regen/despec 额外调用 | 完全对齐 |
| **C_attn** | **Eq(12) `Σ(p²+gp+g(g-1)/2)`** | **`k·B` 简化代理** | **需更新** |
...
| # | 问题 | 严重度 | 修复工作量 |
|---|------|--------|----------|
| 1 | **C_attn 公式不匹配 Eq(12)** — 当前用 `k·B`，Overleaf 要求 `Σ(p²+gp+g(g-1)/2)` | 中 | 中 — 需在 generator 中记录 prompt/gen token 数，更新 trace 格式和 analyze 脚本 |
| 2 | **C_no-cache Eq(11) 未实现** — `Σ(p+g)²` | 低 | 小 — 有了 p_j/g_j 数据后公式计算很简单 |

### 已知差异（可接受 / 需决策）

| # | 差异 | 说明 |
|---|------|------|
| 3 | **惩罚公式**：Overleaf Eq(14) `r̂ - α·sev` vs 实现 `r - γ·ln(1+sev)` | 主 pipeline 用 log-smooth（导师建议改进）；如需对齐可切回线性或更新 Overleaf |
| 4 | **`reroute_scores()` legacy** 用乘法 `S*(1-α*sev)` | 不影响 — 主 pipeline 不调用此函数 |
| 5 | **W_proj 未训练** — identity fallback + spatial routing 绕过 | 等实验数据充足后训练；不影响 workflow 完整性 |
| 6 | **R6 跨句规则** — 实现中有但 Overleaf 未写 | 属于增强，可后续加入 Overleaf 或保持 code-only |
